...
import { queensMockWithID } from "../../../mocks/queensMock.js";
...
describe("Given a DELETE method with the path '/queens/id'", () => {
  beforeAll(async () => {
    await Queen.create(queensMockWithID);
  });
...
describe("Given a GET method with the path '/queens/id'", () => {
  beforeAll(async () => {
    await Queen.findById(queensMockWithID);
  });
  describe("When it receives a request with an authorization header and a queen id", () => {
    test("Then it should return the queen and call the response's status method with the statuscode 200", async () => {
      const expectedStatus = statusCode.ok;
      const idQueen = queensMockWithID[0]._id;
      const response = await request(app)
